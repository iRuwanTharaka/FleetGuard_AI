...
interface InspectionContextType {
  ...
  setVehicle: (vehicleId: string, vehicleNumber: string, make: string, model: string, year: number, vehicleType?: string) => void;
  ...
  setAnnotatedImages: (images: AnnotatedImage[]) => void;
...
export function InspectionProvider({ children }: { children: ReactNode }) {
  ...
  const setVehicle = (vehicleId: string, vehicleNumber: string, make: string, model: string, year: number, vehicleType?: string) => {
    const template = vehicleType === 'tuktuk' ? 6 : 8;
    const defaultPhotos: PhotoData[] = Array.from({ length: template }, (_, i) => ({
      id: String(i + 1),
      label: ['Front View', 'Rear View', 'Left Side', 'Right Side', 'Interior', 'Dashboard/Odometer', 'Damage Close-up', 'Additional'][i] || 'Photo',
      file: null,
      preview: null,
      captured: false,
    }));
    if (template === 6) {
      defaultPhotos[4] = { id: '5', label: 'Engine Compartment', file: null, preview: null, captured: false };
      defaultPhotos[5] = { id: '6', label: 'Passenger Canopy', file: null, preview: null, captured: false };
    }
    setInspection(prev => ({
      ...prev,
      vehicleId,
  ...
  const setAnnotatedImages = (images: AnnotatedImage[]) => {
    setInspection(prev => ({ ...prev, annotatedImages: images }));
  };
